...
// loadSpec reads every per-domain spec under api/openapi/ and merges their
// paths into one document. The contract was split from a single openapi.yaml
// into five bounded-context files (auth/management/federation/developer/
// operations); coverage is the UNION of all of them.
func loadSpec(t *testing.T) specDoc {
	t.Helper()
	// platform/api/rest -> ../../../api/openapi
	dir := filepath.Join("..", "..", "..", "api", "openapi")
	files, err := filepath.Glob(filepath.Join(dir, "*.yaml"))
	if err != nil {
		t.Fatalf("glob %s: %v", dir, err)
	}
	if len(files) == 0 {
		t.Fatalf("no OpenAPI specs found under %s", dir)
	}
	...
	for _, path := range files {
		...
		for p, methods := range doc.Paths {
			if _, dup := merged.Paths[p]; dup {
				t.Fatalf("path %q documented in more than one spec file (expected disjoint)", p)
			}
...
func TestOpenAPICoversAllMountedRoutes(t *testing.T) {
	...
	t.Logf("router mounts %d documented routes; api/openapi/ documents %d", len(mounted), len(spec))

	if len(missing) > 0 {
		t.Errorf("%d mounted route(s) are NOT documented in api/openapi/:\n%s",
