...
- **Roadmap board:** "Qeet ID - Roadmap" = org `qeetgroup` **Project #24** ([github.com/orgs/qeetgroup/projects/24](https://github.com/orgs/qeetgroup/projects/24)) — tracks work PRD → prod via an 8-step Status Kanban (`📥 Backlog → 🔍 Spec & Design → 📋 Ready → 🚧 In Progress → 👀 In Review → 🧪 QA / Staging → 🚀 Release → ✅ Done — In Prod`). Board fields: Priority (P0–P3), Area, **Work Type** (not "Type" — reserved), Workstream, Size (=effort), Target date, FP Ref, Blocked.
- **Issues** live in `qeetgroup/qeet-id`. Convention: title `[feat] …` (or `[fix]`/`[chore]`); body is **Context / Requirements / Acceptance criteria** (checkbox ACs, no "References" section). Labels: `P0`–`P3` + `area/*` + `type/*` + `ws/*`; assign a milestone (`v1.0 — GA` / `v1.1 — Agent & MCP Fast-Follow` / `v1.2 — Standards & Federation` / `Infra & Deploy` / `Ops & Go-Live Hardening` / `Post-GA Backlog`).
- The **`issue-tracker`** subagent ([.claude/agents/issue-tracker.md](.claude/agents/issue-tracker.md), stage 1.5 in [.claude/PIPELINE.md](.claude/PIPELINE.md)) creates these issues + sets board fields, and reconciles the board against the code. `gh` needs the `project` scope (`gh auth refresh -s project`).
