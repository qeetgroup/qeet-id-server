-- 0024_indexes — extra indexes for hot query paths missed by earlier migrations (per-index rationale below)
...
-- Per-tenant active-session list: the existing idx_sessions_active is keyed by
-- user_id, but admin queries filter by tenant first.
...
-- Webhook dispatcher hot path: pick pending deliveries for a specific subscription.
-- The existing idx_webhook_deliv_pending is keyed only by next_attempt_at and scans across all subscriptions.
