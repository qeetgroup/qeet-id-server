...
```mermaid
flowchart TB
    ext["Browsers · IdPs · SDKs · service accounts · AI agents"]

    subgraph clients["Frontends"]
        direction LR
        login["login<br/>Next.js"]
        console["console<br/>Vite admin"]
        website["website<br/>Next.js"]
    end

    subgraph backend["Go API · cmd/server · chi v5 — single deployable"]
        direction TB
        mw["Middleware: RequestID → RealIP → Recoverer → SecurityHeaders → AccessLog<br/>→ Tracing → Metrics → CSRF → CORS → authz"]
        domains["domains/ — identity · access · federation · developer · operations<br/>(interface-mediated calls)"]
        platform["platform/ — api · database · cache · messaging · events<br/>observability · security · config"]
        mw --> domains --> platform
    end

    pg[("PostgreSQL · pgx v5<br/>6 schemas · multi-tenant by tenant_id")]
    egress["Egress: SMTP · webhooks · SIEM · HIBP<br/>transactional outbox · at-least-once"]

    ext --> login & console & website
    login --> mw
    console --> mw
    website --> mw
    platform --> pg
