...
tests/      Go integration tests        tools/ codegen + scripts + benchmarks
```
...
Single HTTP entrypoint [cmd/server/main.go](cmd/server/main.go) (worker/scheduler/migrate are sibling entrypoints under [cmd/](cmd/)); HTTP wiring in [platform/api/rest/router.go](platform/api/rest/router.go) (chi v5). Persistence is PostgreSQL via pgx v5, **everything multi-tenant by `tenant_id`** across `tenant`/`user`/`auth`/`rbac`/`audit`/`platform` schemas. Config is envconfig-driven ([platform/config/config.go](platform/config/config.go)); `HTTP_PORT` defaults to `4001`. Event publishing uses a transactional outbox + webhook dispatcher with DLQ; the audit log is hash-chained/append-only. Migrations are golang-migrate SQL files in [platform/database/migrations/](platform/database/migrations/) (apply via `make migrate-up`, never edit an applied migration — add a new pair). API contract: [api/openapi/](api/openapi/) — five bounded-context OpenAPI 3.1 specs, no monolith (merge with `go run ./tools/openapi-split merge`); CI guard reads their union — plus a Postman collection exercised by `make test-api`.
...
- **Docker:** `Dockerfile` is at the repo root — `docker build -t qeet-id:latest .`. The [.dockerignore](.dockerignore) excludes the JS workspace; migration SQL files are embedded in the binary via `//go:embed` in `platform/database/migrations/runner.go` and run automatically on startup.
