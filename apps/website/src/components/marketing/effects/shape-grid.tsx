 ...
 * Adapted from React Bits "ShapeGrid" for this codebase:
 *  - theme-aware default colors (reads the `.dark` class on <html>),
 *  - reduced-motion safe (renders a single static frame, no rAF loop),
 *  - pauses the animation loop when scrolled out of view (IntersectionObserver).
 * Colors default to brand-tinted neutrals; pass explicit props to override.
 */
...
export interface ShapeGridProps {
  ...
  /** When set, paints a radial vignette fading to this color at the edges. Omit to fade via a CSS mask on the wrapper instead. */
  vignetteColor?: string;
  className?: string;
  style?: CSSProperties;
}

/** Track the `.dark` class on <html> so canvas colors follow the active theme. */
function useIsDark() {
...
export function ShapeGrid({
  ...
  const resolvedBorder: CanvasStrokeStyle =
    borderColor ?? (isDark ? "rgba(255,255,255,0.07)" : "rgba(15,15,20,0.07)");
  const resolvedHoverFill: CanvasStrokeStyle =
    hoverFillColor ?? (isDark ? "rgba(242,109,14,0.16)" : "rgba(242,109,14,0.12)");
  ...
  useEffect(() => {
    ...
    if (reduce) {
      // Static single frame — no drift, no hover animation.
      drawGrid();
    } else {
      ...
      io = new IntersectionObserver(
        ([entry]) => {
          ...
          if (visible && requestRef.current == null) {
            requestRef.current = requestAnimationFrame(updateAnimation);
          }
        },
        { threshold: 0 },
