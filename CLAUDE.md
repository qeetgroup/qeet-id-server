...
Only packages with real code exist; planned work lives in [ROADMAP.md](ROADMAP.md), not empty dirs. The former semantic folder≠package mismatches were aligned to **folder == package** (`organizations`→`tenant`, `compliance`→`gdpr`, `identity/domains`→`domainverify`, `service-accounts`→`principal`). **A few folders still differ where the package uses a shorter idiomatic name — intentional and legal** (e.g. `internal/access/authentication` = `package auth`, `internal/developer/api-keys` = `package apikey`, `internal/operations/email` = `package emailtemplate`, `internal/platform/jobs` = `package worker`).
