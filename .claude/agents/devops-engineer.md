...
- **Images:** root `Dockerfile` (distroless app; `COPY . .` from repo root + `.dockerignore`; build-args `VERSION/COMMIT/BUILD_DATE` → `platform/observability/buildinfo` ldflags) and `Dockerfile.migrate` (`COPY migrations /migrations`).
