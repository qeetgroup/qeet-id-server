...
| Self-service signup (org signup) | ✅ | [backend/internal/auth/service.go](../backend/internal/auth/service.go), [backend/internal/auth/http.go](../backend/internal/auth/http.go) | `POST /v1/auth/signup` public, rate-limited. In one tx: creates **tenant + user + system `owner` role with all permissions + role assignment + session**. Returns `{user, tenant, tenant_id, user_id, roles:["owner"], access_token, refresh_token}`. The signup user is the tenant admin and can manage everything in their tenant. They can also create additional tenants via `POST /v1/tenants`. |
